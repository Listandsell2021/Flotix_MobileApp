...
const ExpenseFormScreen: React.FC<ExpenseFormScreenProps> = ({ navigation, route }) => {
  const { state, addExpense, setLoading, setError } = useExpense();
  const { state: authState } = useAuth();
  const { receiptUrl } = route.params;
  
  // Get the vehicle's current odometer from driver data
  const vehicleOdometer = authState.driverData?.assignedVehicle?.currentOdometer || 0;
  ...
  const validateForm = () => {
    ...
    if (!formData.odometerReading || formData.odometerReading.trim() === '') {
      ...
      if (isNaN(currentReading) || currentReading < 0) {
        newErrors.odometerReading = 'Odometer reading must be a valid positive number';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (receiptUrl && (receiptUrl.startsWith('file://') || receiptUrl.startsWith('/'))) {
        try {
          ...
          if (uploadError.message?.includes('404')) {
            ...
            console.log('Using mock URL (upload endpoint not available):', uploadedReceiptUrl);
          } else {
            // Other error, use placeholder
            uploadedReceiptUrl = 'https://via.placeholder.com/400x600/cccccc/666666?text=Receipt';
            showToast('Receipt upload failed, using placeholder', 'error');
          }
        }
        ...
        uploadedReceiptUrl = 'https://via.placeholder.com/400x600/cccccc/666666?text=No+Receipt';
      }
      ...
      const expenseData: any = {
        ...
        odometerReading: formData.odometerReading ? parseFloat(formData.odometerReading) : vehicleOdometer || 0,
      };
      ...
      const errorMessage = 
        error instanceof Error ? error.message : 'Failed to create expense. Please try again.';
        ...
        <View style={styles.content}>
          {receiptUrl && (
            <View style={styles.receiptCard}>
              ...
              <ReceiptPreview uri={receiptUrl} showFullSize style={styles.receiptPreview} />
              ...
              {expenseTypes.map(type => (
                ...
                >
                  <Icon 
                    ...
                    size={16} 
                    color={formData.type === type.value ? theme.colors.primary : theme.colors.textSecondary} 
                  />
                  <Text
                    style={[
          ...
          <View style={styles.sectionCard}>
            ...
            <TextInput
              ...
              placeholder={formData.type === 'Fuel' ? "Shell Gas Station" : "Store Name"}
          ...
          <View style={styles.sectionCard}>
            ...
            <TextInput
              ...
              placeholder={vehicleOdometer ? `Previous: ${vehicleOdometer} km` : "Enter current reading"}
              error={errors.odometerReading}
            />
            
            {vehicleOdometer > 0 && formData.odometerReading && parseFloat(formData.odometerReading) > vehicleOdometer && (
              <View style={styles.distanceCard}>
                <Icon name="location" size={16} color={theme.colors.success} />
                <Text style={styles.distanceText}>
                  Distance traveled: {(parseFloat(formData.odometerReading) - vehicleOdometer).toFixed(0)} km
                </Text>
              </View>
            )}
          </View>
